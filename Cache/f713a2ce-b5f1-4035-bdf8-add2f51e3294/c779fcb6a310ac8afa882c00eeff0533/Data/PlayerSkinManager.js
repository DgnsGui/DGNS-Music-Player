...
function validateInputs() {
    ...
    // Also check button script components later in bindButton
    return valid;
}
...
function switchToSkin(newSkinName) {
    ...
    if (newSkinData.manager) {
        ...
        // Example:
        // var managerScript = newSkinData.manager.getComponent("Component.ScriptComponent");
        // if (managerScript && managerScript.api && typeof managerScript.api.resumeTrack === 'function') {
        //     managerScript.api.resumeTrack();
        // }
    }
...
function bindButton(buttonScriptComponent, buttonName, callback) {
    ...
    // Access the actual PinchButton script/API (assuming it exposes onButtonPinched)
    // Sometimes the API is directly on the ScriptComponent, sometimes nested.
    // Adjust if your PinchButton script uses a different API structure.
    var pinchButtonAPI = buttonScriptComponent.api ? buttonScriptComponent.api : buttonScriptComponent;

    if (!pinchButtonAPI || typeof pinchButtonAPI.onButtonPinched !== 'object' || typeof pinchButtonAPI.onButtonPinched.add !== 'function') {
        print("ERROR: PlayerSkinManager - ScriptComponent for '" + buttonName + "' does not have a valid 'onButtonPinched' event. Ensure it's a PinchButton script or similar.");
        // You might need to inspect the `buttonScriptComponent` in the logger to see its available properties/api.
        // print("Debug Info for " + buttonName + ":");
        // for (var key in buttonScriptComponent) { print("  " + key); }
        // if(buttonScriptComponent.api) for (var key in buttonScriptComponent.api) { print("  api." + key); }
        return;
    }

    // Add the listener
    try {
        pinchButtonAPI.onButtonPinched.add(function() {
            print("EVENT: Button pinched: " + buttonName + " (Current Skin: " + currentSkin + ")");
            // Check if the button press is relevant for the current skin to avoid unintended triggers
            // (e.g., if somehow a disabled button's event fired)
