...
function bindHandlers() {
    ...
    if (script.acknowledgeButton && script.acknowledgeButton.api && script.acknowledgeButton.api.onButtonPinched) {
        script.acknowledgeButton.api.onButtonPinched.add(function() {
            print("Acknowledge button pressed.");
            switchToY2K();
        });
    } else {
        print("Error: Acknowledge button missing proper API or onButtonPinched property!");
    }

    // Y2K skin buttons
    if (script.y2kSkinButton && script.y2kSkinButton.api && script.y2kSkinButton.api.onButtonPinched) {
        script.y2kSkinButton.api.onButtonPinched.add(function() {
            print("Y2K skin button pressed.");
            switchToModern();
        });
    } else {
        print("Error: Y2K skin button missing proper API or onButtonPinched property!");
    }
    
    if (script.y2kHandButton && script.y2kHandButton.api && script.y2kHandButton.api.onButtonPinched) {
        script.y2kHandButton.api.onButtonPinched.add(function() {
            print("Y2K hand button pressed.");
            switchToHand();
        });
    } else {
        print("Error: Y2K hand button missing proper API or onButtonPinched property!");
    }

    // Modern skin buttons
    if (script.modernSkinButton && script.modernSkinButton.api && script.modernSkinButton.api.onButtonPinched) {
        script.modernSkinButton.api.onButtonPinched.add(function() {
            print("Modern skin button pressed.");
            switchToY2K();
        });
    } else {
        print("Error: Modern skin button missing proper API or onButtonPinched property!");
    }
    
    if (script.modernHandButton && script.modernHandButton.api && script.modernHandButton.api.onButtonPinched) {
        script.modernHandButton.api.onButtonPinched.add(function() {
            print("Modern hand button pressed.");
            switchToHand();
        });
    } else {
        print("Error: Modern hand button missing proper API or onButtonPinched property!");
    }

    // Hand skin buttons
    if (script.handSkinButton && script.handSkinButton.api && script.handSkinButton.api.onButtonPinched) {
        ...
        print("Error: Hand skin button missing proper API or onButtonPinched property!");
    }
    
    if (script.handHandButton && script.handHandButton.api && script.handHandButton.api.onButtonPinched) {
        script.handHandButton.api.onButtonPinched.add(function() {
            print("Hand hand button pressed.");
            returnToPreviousSkin();
        });
    } else {
        print("Error: Hand hand button missing proper API or onButtonPinched property!");
    }
}
