...
function disableAllSkins() {
    ...
    // Disable all button SceneObjects to prevent overlap
    if (script.y2kSkinButton) script.y2kSkinButton.getSceneObject().enabled = false;
    if (script.y2kHandButton) script.y2kHandButton.getSceneObject().enabled = false;
    if (script.modernSkinButton) script.modernSkinButton.getSceneObject().enabled = false;
    if (script.modernHandButton) script.modernHandButton.getSceneObject().enabled = false;
    if (script.handSkinButton) script.handSkinButton.getSceneObject().enabled = false;
    if (script.handHandButton) script.handHandButton.getSceneObject().enabled = false;
}
...
function enableSkin(skin) {
    ...
    switch (skin) {
        ...
        case SKINS.Y2K:
            ...
            if (script.y2kSkinButton) script.y2kSkinButton.getSceneObject().enabled = true;
            if (script.y2kHandButton) script.y2kHandButton.getSceneObject().enabled = true;
            break;
        case SKINS.MODERN:
            script.musicPlayerModern.enabled = true;
            script.musicPlayerManagerModern.enabled = true;
            if (script.modernSkinButton) script.modernSkinButton.getSceneObject().enabled = true;
            if (script.modernHandButton) script.modernHandButton.getSceneObject().enabled = true;
            break;
        case SKINS.HAND:
            script.musicPlayerHand.enabled = true;
            script.musicPlayerManagerHand.enabled = true;
            if (script.handSkinButton) script.handSkinButton.getSceneObject().enabled = true;
            if (script.handHandButton) script.handHandButton.getSceneObject().enabled = true;
            break;
    }
}
...
function bindHandlers() {
    ...
    // Modern buttons - Fixed to properly toggle between Y2K and Hand
    bindButton(script.modernSkinButton, "Modern Skin", function() {
        switchSkin(SKINS.Y2K);  // Correctly switch to Y2K skin
    });
    bindButton(script.modernHandButton, "Modern Hand", function() {
        switchSkin(SKINS.HAND); // Correctly switch to Hand skin
    });

    // Hand buttons
    bindButton(script.handSkinButton, "Hand Skin", function() {
        // If coming from Modern, go back to Modern, otherwise go to Y2K
        switchSkin(previousSkin === SKINS.MODERN ? SKINS.MODERN : SKINS.Y2K);
    });
    bindButton(script.handHandButton, "Hand Hand", function() {
        // Go back to previous skin, default to Y2K if no previous
        switchSkin(previousSkin || SKINS.Y2K);
    });
}

// Helper to bind a button with detailed error checking
function bindButton(button, name, callback) {
    if (!button) {
        print("Error: " + name + " button is not assigned in Inspector!");
        return;
    }
    if (!button.getSceneObject) {
        print("Error: " + name + " is not a valid ScriptComponent! Assigned: " + button);
        return;
    }
    const sceneObjectName = button.getSceneObject().name || "unnamed";
    if (button.onButtonPinched) {
        button.onButtonPinched.add(callback);
        print(name + " button bound successfully. SceneObject: " + sceneObjectName);
    } else {
        print("Error: " + name + " lacks onButtonPinched! SceneObject: " + sceneObjectName + ". Ensure it has a PinchButton script.");
    }
}
