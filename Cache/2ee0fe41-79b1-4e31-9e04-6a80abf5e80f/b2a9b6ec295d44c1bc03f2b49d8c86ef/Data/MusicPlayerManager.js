...
// Variable pour suivre l'état des pistes audio
var trackFinishedState = [];
...
function initialize() {
    ...
    // Au lieu d'utiliser onFinish.add, on utilise un UpdateEvent pour vérifier l'état de lecture
    var updateEvent = script.createEvent("UpdateEvent");
    updateEvent.bind(checkTrackFinished);
    
    // Commencer la mise à jour de la barre de progression
    var progressUpdateEvent = script.createEvent("UpdateEvent");
    progressUpdateEvent.bind(updateProgress);
}

// Nouvelle fonction pour vérifier si une piste est terminée
function checkTrackFinished() {
    if (isPlaying && currentTrackIndex < audioComponents.length) {
        ...
        // Si la piste était en lecture mais ne l'est plus, et n'est pas en pause
        if (!currentTrack.isPlaying() && !currentTrack.isPaused()) {
            // Vérifions que la piste a bien été jouée (temps proche de la fin)
            if (currentTrack.getTime() > 0) {
                // Marquer comme terminée si ce n'est pas déjà fait
                if (!trackFinishedState[currentTrackIndex]) {
                    trackFinishedState[currentTrackIndex] = true;
                    onTrackFinished();
                }
            }
        } else {
            trackFinishedState[currentTrackIndex] = false;
        }
    }
}

function setupButtonListeners() {
    // Méthode de configuration des écouteurs d'événements pour les boutons PinchButton
    
    // Configuration du bouton play/pause
    if (script.playPauseButton) {
        script.playPauseButton.api.customFunctionForOnButtonPinched = script;
        script.playPauseButton.api.onButtonPinchedFunctionNames = ["togglePlayPause"];
    }
    
    // Configuration du bouton next track
    if (script.nextTrackButton) {
        script.nextTrackButton.api.customFunctionForOnButtonPinched = script;
        script.nextTrackButton.api.onButtonPinchedFunctionNames = ["nextTrack"];
    }
    
    // Configuration du bouton back track
    if (script.backTrackButton) {
        script.backTrackButton.api.customFunctionForOnButtonPinched = script;
        script.backTrackButton.api.onButtonPinchedFunctionNames = ["previousTrack"];
    }
    
    // Configuration du bouton shuffle
    if (script.shuffleButton) {
        script.shuffleButton.api.customFunctionForOnButtonPinched = script;
        script.shuffleButton.api.onButtonPinchedFunctionNames = ["toggleShuffle"];
    }
    
    // Configuration du bouton repeat
    if (script.repeatButton) {
        script.repeatButton.api.customFunctionForOnButtonPinched = script;
        script.repeatButton.api.onButtonPinchedFunctionNames = ["toggleRepeat"];
    }
    
    // Configuration du bouton stop
    if (script.stopButton) {
        script.stopButton.api.customFunctionForOnButtonPinched = script;
        script.stopButton.api.onButtonPinchedFunctionNames = ["stopTrack"];
    }
}
