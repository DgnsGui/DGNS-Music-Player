...
let MusicPlayerManager = class MusicPlayerManager extends BaseScriptComponent {
    ...
    setupCallbacks() {
        ...
        if (this.repeatButton) {
            this.onRepeatCallback = (event) => {
                const ct = getTime();
                if (ct - this.lastPinchTimeRepeat < this.DEBOUNCE_TIME)
                    return;
                this.lastPinchTimeRepeat = ct; // Add debounce
        ...
        if (this.shuffleButton) {
            this.onShuffleCallback = (event) => {
                const ct = getTime();
                if (ct - this.lastPinchTimeShuffle < this.DEBOUNCE_TIME)
                    return;
                this.lastPinchTimeShuffle = ct; // Add debounce
        ...
        if (this.stopButton) {
            this.onStopCallback = (event) => {
                ...
                if (ct - this.lastPinchTimeStop < this.DEBOUNCE_TIME)
                    return;
                this.lastPinchTimeStop = ct; // Add debounce
                this.stopTrack();
            };
        ...
        this.onTrackFinishedCallback = (audioComponent) => {
            ...
            if (audioComponent !== this.audioComponent || !this.isPlaying || this.isLoadingRemote || this.isManualStop || this.isPaused) {
                if (this.isManualStop)
                    this.isManualStop = false; // Reset flag if it was set
    ...
    loadTrack(index) {
        ...
        if (trackData.isRemote) {
            ...
            const onDownloadedCallback = (downloadedAsset) => {
                ...
                if (downloadedAsset && downloadedAsset.isOfType("Asset.AudioTrackAsset")) {
                    ...
                    try {
                        ...
                        this.currentPlaybackTime = 0; // Reset time on successful load
                        if (playAfterLoad) {
            ...
            try {
                ...
                this.audioInitialized = true;
                this.trackStartTime = getTime();
                this.currentPlaybackTime = 0; // Reset time
    ...
    pauseTrack() {
        ...
        if (!this.isPlaying || !this.audioInitialized || !this.audioComponent || !this.audioComponent.isPlaying()) {
            print(`Pause ignored: Not in a valid playing state (Playing:${this.isPlaying}, Init:${this.audioInitialized}, CompPlaying:${(_a = this.audioComponent) === null || _a === void 0 ? void 0 : _a.isPlaying()})`);
    ...
    nextTrack() {
        ...
        if (this.isRepeatEnabled && currentIndex !== -1) {
            ...
            if (this.allTracksData.length > 1) {
                ...
                nextIndex = 0;
            } // Only one track, shuffle does nothing
