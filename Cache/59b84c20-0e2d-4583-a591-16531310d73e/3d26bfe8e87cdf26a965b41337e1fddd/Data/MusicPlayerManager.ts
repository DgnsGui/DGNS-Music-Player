...
export class MusicPlayerManager extends BaseScriptComponent {
    ...
    private setupCallbacks(): void {
        ...
        if (this.repeatButton) {
            this.repeatButton.onButtonPinched((event: InteractorEvent) => {
        ...
        if (this.shuffleButton) {
            this.shuffleButton.onButtonPinched((event: InteractorEvent) => {
        ...
        if (this.stopButton) {
            this.stopButton.onButtonPinched((event: InteractorEvent) => this.stopTrack());
        }
    ...
    private stopTrack(): void {
        if (this.audioComponent) {
            ...
            this.isPlaying = false;
            this.isPaused = false;
            this.shouldAutoPlay = false; // Empêche le déclenchement automatique d'une autre piste
            this.currentPlaybackTime = 0;
            this.updateEarthPosition(0, 1);
    ...
    private nextTrack(): void {
        if (this.isRepeatEnabled) {
            // Si Repeat est activé, redémarrer la piste actuelle
