        ...
        if (this.earthSphere && this.progressBar && totalTime > 0) {
            try {
                ...
                if (isChildOfProgressBar) {
                    ...
                    // Si ce n'est pas un enfant, utiliser une approche basée sur l'espace monde
                    // mais tenant compte de la rotation de la barre
                    
                    // Obtenir l'échelle et la position de la barre
                    const barScale = barTransform.getLocalScale();
                    const barPosition = barTransform.getWorldPosition();
                    
                    // Calculer le vecteur de direction de la barre (axe X local transformé en espace monde)
                    const barDirection = barTransform.transformDirection(new vec3(1, 0, 0));
                    
                    // Normaliser la direction
                    const normalizedDirection = barDirection.normalize();
                    
                    // Calculer le point de départ (extrémité gauche de la barre)
                    const startPoint = new vec3(
                        barPosition.x - normalizedDirection.x * (barScale.x / 2),
                        barPosition.y - normalizedDirection.y * (barScale.x / 2),
                        barPosition.z - normalizedDirection.z * (barScale.x / 2)
                    );
                    
                    // Calculer la nouvelle position basée sur la progression
                    const newPosition = new vec3(
                        startPoint.x + normalizedDirection.x * barScale.x * progress + this.earthSphereXOffset * normalizedDirection.x,
                        startPoint.y + normalizedDirection.y * barScale.x * progress + this.earthSphereXOffset * normalizedDirection.y,
                        startPoint.z + normalizedDirection.z * barScale.x * progress + this.earthSphereXOffset * normalizedDirection.z
                    );
