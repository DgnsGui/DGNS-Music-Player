...
let MusicPlayerManager = class MusicPlayerManager extends BaseScriptComponent {
    ...
    setupProgressBar() {
        if (this.progressBar && this.earthSphere) {
            // Aligner la sphère avec la position de la barre dans l'espace XYZ
            ...
            // Définir la position de la sphère pour qu'elle soit au centre de la barre
            earthTransform.setLocalPosition(new vec3(barPosition.x, barPosition.y, barPosition.z));
            // Initialiser la position de la sphère à 0% de progression
    ...
    updateEarthPosition(currentTime, totalTime) {
        if (this.earthSphere && this.progressBar && totalTime > 0) {
            try {
                ...
                // Obtenir les transformations de la barre et de la sphère
                const barTransform = this.progressBar.getTransform();
                const barScale = barTransform.getLocalScale();
                const barPosition = barTransform.getLocalPosition();
                const earthTransform = this.earthSphere.getTransform();
                // Calculer la position X en fonction de la progression
                const halfLength = barScale.x / 2;
                const localX = -halfLength + (progress * barScale.x) + this.earthSphereXOffset;
                // Aligner la sphère avec la barre sur les axes Y et Z, et ajuster X
                earthTransform.setLocalPosition(new vec3(barPosition.x + localX, // Ajuster X en fonction de la progression
                barPosition.y, // Même Y que la barre
                barPosition.z // Même Z que la barre
                ));
                // Appliquer la rotation de la sphère
