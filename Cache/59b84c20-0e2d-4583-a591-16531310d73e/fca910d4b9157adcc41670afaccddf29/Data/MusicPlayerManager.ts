...
export class MusicPlayerManager extends BaseScriptComponent {
    ...
    private isManualStop: boolean = false; // Nouveau drapeau pour arrêt manuel
    ...
    private handleTrackFinished(): void {
        if (this.isManualStop) {
            ...
            this.isManualStop = false; // Réinitialiser après usage
            return;
        }

        if (this.currentTrackIndex === -1) {
            return; // Ne rien faire si aucune piste n'est chargée
        }
        ...
        if (this.isRepeatEnabled) {
            ...
            this.stopTrack(); // Arrêter complètement si on atteint la fin sans boucle
        }
    }
    ...
    private loadTrack(index: number): void {
        if (index >= 0 && index < this.tracks.length) {
            ...
            if (this.isPlaying) {
                this.isManualStop = true; // Indiquer un arrêt manuel
    ...
    private togglePlayPause(): void {
        if (this.isPlaying) {
            ...
            if (this.currentTrackIndex === -1) {
                // Charger la première piste si aucune n'est chargée
                this.loadTrack(0);
    ...
    private stopTrack(): void {
        if (this.audioComponent) {
            ...
            this.isPaused = false;
            this.shouldAutoPlay = false;
            this.currentTrackIndex = -1; // État "arrêté" : aucune piste chargée
            this.audioComponent.audioTrack = null; // Décharger la piste
            this.audioInitialized = false;
            this.currentPlaybackTime = 0;
            this.updateEarthPosition(0, 1);
            this.updateTrackInfo();
            ...
            if (this.isShuffleEnabled) {
                ...
                if (nextIndex >= this.tracks.length) {
                    if (this.loopPlayback) {
                        nextIndex = 0; // Revenir au début si boucle activée
                    } else {
                        this.stopTrack(); // Arrêter si fin de liste sans boucle
                        return;
                    }
                }
            }
    ...
    private prevTrack(): void {
        if (this.currentTrackIndex === -1) {
            // Si aucune piste n'est chargée, charger la dernière
        ...
        if (this.isRepeatEnabled) {
            ...
            if (this.isShuffleEnabled) {
                ...
                if (prevIndex < 0) {
                    prevIndex = this.tracks.length - 1; // Aller à la dernière piste si au début
                }
            }
