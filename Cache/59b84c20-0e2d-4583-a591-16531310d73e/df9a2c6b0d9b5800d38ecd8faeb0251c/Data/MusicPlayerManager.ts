...
// Déclarer le décorateur @component (implicitement fourni par Lens Studio)
declare function component(target: any): void;

// Déclarer une interface minimale pour la classe de base implicite de Lens Studio
interface LensStudioScript {
    createEvent(eventType: string): {
        bind(callback: () => void): void;
        enabled: boolean;
    };
    // Ajoutez d'autres méthodes si nécessaire (ex. getSceneObject, getTransform)
}

// Appliquer l'interface à la classe sans forcer un héritage explicite incompatible
@component
export class OptimizedMusicPlayer implements LensStudioScript {
    ...
    createEvent(eventType: string): { bind(callback: () => void): void; enabled: boolean } {
