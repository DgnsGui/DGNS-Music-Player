...
interface TrackData {
    asset: AudioTrackAsset | Asset.RemoteReferenceAsset;
    artist: string;
    title: string;
...
export class MusicPlayerManager extends BaseScriptComponent {
    ...
    remoteTracks: Asset.RemoteReferenceAsset[];
    ...
    @input(' dehydratedComponent.ScriptComponent') // Typo in original? Assuming Component.ScriptComponent
        ...
        if (this.localTracks) {
            for (let i = 0; i < this.localTracks.length; i++) {
                this.allTracksData.push({
                    ...
                    artist: this.localArtists[i] || "Unknown Artist",
                    title: this.localTitles[i] || "Unknown Title",
                    prefab: this.localTrackPrefabs[i] || null,
                    isRemote: false
                });
            }
        }
        ...
        if (this.remoteTracks) {
             for (let i = 0; i < this.remoteTracks.length; i++) {
                this.allTracksData.push({
                    ...
                    artist: this.remoteArtists[i] || "Unknown Artist",
                    title: this.remoteTitles[i] || "Unknown Title",
                    prefab: this.remoteTrackPrefabs[i] || null,
                    isRemote: true
                });
            }
        }
    ...
    private loadTrack(index: number): void {
        ...
        if (trackData.isRemote) {
            ...
            const remoteAsset = trackData.asset as Asset.RemoteReferenceAsset;

            // Update UI to indicate loading (optional, e.g., change text)
            if (this.timecodeText) this.timecodeText.text = "Loading...";

            remoteAsset.downloadAsset((downloadedAsset: Asset) => {
                // --- Download Succeeded ---
                 // Check if the loaded track is still the intended one
                if (this.currentTrackIndex !== index || !this.isLoadingRemote) {
                    ...
                    // Potentially clean up the downloaded asset if Lens Studio doesn't manage it?
                    return;
                }
                ...
                if (downloadedAsset && downloadedAsset.isOfType("Asset.AudioTrackAsset")) {
                    ...
                    if (wasPlaying) {
                        ...
                        this.playTrack(); // Play if it was intended
                    } else {
                        print("Download complete, track ready.");
                         // Update timecode even if not playing yet
                        this.updatePlayer();
                    }
                } else {
                    print(`Error: Downloaded asset for track ${index} is not an AudioTrackAsset.`);
        ...
        if (this.isPlaying) {
            this.pauseTrack();
             ...
             if (this.currentTrackIndex === -1) {
                ...
                this.loadTrack(0);
                // playTrack will be called by loadTrack if shouldAutoPlay is true and loading succeeds
    ...
    private playTrack(): void {
     ...
     private pauseTrack(): void {
        ...
        if (!this.isPlaying || !this.audioComponent || !this.audioComponent.isPlaying()) {
            print("Pause called but not currently playing.");
            // Ensure states are correct if API call failed previously
            this.isPlaying = false;
             // Keep isPaused true if it was already paused? Or set to false?
             // Let's ensure isPlaying is false. The user intended pause, maybe it's already paused.
            return;
        }
            ...
            // Check AudioComponent's state before calling stop
             if (this.audioComponent.isPlaying() || this.audioComponent.isPaused()) {
                ...
                this.audioComponent.stop(true); // Stop with fade if possible/configured
                 ...
                 print("AudioComponent was not playing or paused.");
             }
    ...
    private updateTrackInfo(): void {
     ...
     private updatePlayer(): void {
        // Don't update timecode if loading remote track
        if (this.isLoadingRemote) {
            if (this.timecodeText && this.timecodeText.text !== "Loading...") {
                 this.timecodeText.text = "Loading..."; // Ensure loading text stays
            }
             // Keep sphere at start while loading
         ...
         if (!this.audioInitialized || !this.audioComponent || !this.audioComponent.audioTrack) {
             // Show 00:00 / 00:00 if stopped or not ready
            if (this.timecodeText) {
                // Avoid flickering if already showing stopped state
                const stoppedText = "00:00 / 00:00";
                if (this.timecodeText.text !== stoppedText) {
                    this.timecodeText.text = stoppedText;
                }
            }
        ...
        if (this.timecodeText) {
            ...
            try {
                ...
                if (isNaN(totalTime) || totalTime <= 0) {
                    // Still waiting for duration? Show 0 or keep previous?
                    // Let's show 0 to indicate it's not ready or invalid
                     totalTime = 0;
                     // If currently playing, still try to get position
                     if (this.isPlaying) {
                    ...
                    // Duration is valid, calculate current time
                    if (this.isPlaying) {
                        currentTime = getTime() - this.trackStartTime;
                        // Clamp currentTime to duration
