...
// Removed problematic import: import { TransformEventArg } from 'SpectaclesInteractionKit/Core/Interaction/InteractionEventArg';
...
export class MusicPlayerManager extends BaseScriptComponent {
    ...
    private onScrubStart(event: any): void { // Using any
        if (this.currentTrackIndex < 0 || !this.audioInitialized || this.isLoadingRemote || !this.audioComponent?.audioTrack || !this.audioComponent.audioTrack.isStreamable()) { // Added streamable check
            print(`Scrub start ignored: Player not ready or track not streamable (Index:${this.currentTrackIndex}, Init:${this.audioInitialized}, Loading:${this.isLoadingRemote}, Streamable:${this.audioComponent?.audioTrack?.isStreamable()}).`);
            // Consider disabling interaction temporarily if needed, though complex.
            return;
        }
        ...
        // Optional: Pause during scrub? Decided against for now.
    }
    ...
    // --- REVISED: Scrub End Handler (No Seek Offset Possible) ---
    private onScrubEnd(event: any): void { // Using any
        ...
        print("Scrub End");
        const wasScrubbing = this.isScrubbing; // Capture state before potentially modifying
        ...
        // Calculate final target time for logging, even though we can't seek to it
        ...
        try {
            ...
            this.isPaused = false;
            this.currentPlaybackTime = 0; // Reset time to 0 as we restart
            this.trackStartTime = getTime(); // Reset start time for accurate updates from beginning
            ...
            if (!this.wasPlayingBeforeScrub) {
                 ...
                 this.delayedCall(0.05, () => {
                     // Check if still valid to pause (haven't stopped/changed track in the meantime)
                     if(this.currentTrackIndex !== -1 && this.isPlaying && !this.isScrubbing) {
                        this.pauseTrack();
                     } else {
                         print("Skipped post-scrub pause (state changed).");
                         this.updatePlayer(); // Ensure UI reflects actual state
                     }
                 });
            } else {
                 this.updatePlayer(); // Update UI immediately after restart
            }
            // --- End Revised Seek ---

        } catch (e) {
            print(`Error during scrub end (stop/play): ${e}`);
            this.stopTrack(); // Recover by stopping
            this.updatePlayer();
        }

         // Force UI update to show correct time (should be near 00:00)
         this.updatePlayer();
    }
    ...
    private updateSphereRotation(): void {
     ...
     private updateButtonVisuals(): void {
         // Example for play/pause button (requires button script API knowledge)
         // if (this.playPauseButton && this.playPauseButton.api.setPressedTexture) { // Hypothetical API
         //     const texture = this.isPlaying ? pauseTexture : playTexture;
         //     this.playPauseButton.api.setPressedTexture(texture);
         // }
         // Example for repeat/shuffle (could enable/disable a child "indicator" object)
         // if (this.repeatButton) {
         //     const indicator = this.repeatButton.getSceneObject().getChild(0); // Assuming indicator is first child
         //     if (indicator) indicator.enabled = this.isRepeatEnabled;
         // }
         // if (this.shuffleButton) {
         //    const indicator = this.shuffleButton.getSceneObject().getChild(0);
         //    if (indicator) indicator.enabled = this.isShuffleEnabled;
         //}
     }
