...
let MusicPlayerManager = class MusicPlayerManager extends BaseScriptComponent {
        ...
        if (this.currentTrackIndex === -1 || this.isLoadingRemote) { // --- MODIFIED: Check loading state ---
            ...
            if (this.stoppedPrefab) {
                ...
                if (this.isLoadingRemote)
                    print("Loading prefab activated.");
                else
    ...
    loadTrack(index) {
        ...
        if (assetToLoad.isOfType("Asset.AudioTrackAsset")) {
            ...
            const remoteAssetRef = assetToLoad;
            ...
            (downloadedAsset) => {
                ...
                const actualAudioTrack = downloadedAsset;
    ...
    handleLoadError(failedIndex, wasPlayingBeforeLoad) {
    ...
    assignAndPrepareAudio(audioTrack, shouldPlayWhenReady) {
        if (!audioTrack) {
    ...
    updateTrackInfo() {
        ...
        if (this.currentTrackIndex === -1 || this.currentTrackIndex >= this.titles.length) {
            ...
            if (this.trackTitleText)
                this.trackTitleText.text = this.isLoadingRemote ? "Loading..." : "Stopped";
        ...
        if (this.earthSphere && this.progressBar && this.earthSphere.enabled && this.progressBar.enabled) {
            ...
            if (totalTime > 0) {
                try {
                    ...
                    earthTransform.setLocalPosition(new vec3(barPosition.x + localX, barPosition.y, // Keep original Y
