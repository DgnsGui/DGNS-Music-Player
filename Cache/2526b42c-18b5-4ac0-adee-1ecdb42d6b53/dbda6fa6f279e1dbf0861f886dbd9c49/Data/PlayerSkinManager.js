...
function toggleSkins() {
    ...
    print("Switched to " + (isModernActive ? "Modern" : "Y2K") + " skin. isModernActive: " + isModernActive);
}
...
function setupPinchEvents() {
    ...
    if (script.y2kPinchButton.onButtonPinched) {
        y2kHandler = function() {
            print("Y2K Pinch Button pressed. isModernActive: " + isModernActive);
            if (!isModernActive) { // Only toggle if Y2K is active
                toggleSkins();
            } else {
                print("Y2K Pinch Button ignored: Modern skin is already active.");
            }
        };
    ...
    if (script.modernPinchButton.onButtonPinched) {
        modernHandler = function() {
            print("Modern Pinch Button pressed. isModernActive: " + isModernActive);
            if (isModernActive) { // Only toggle if Modern is active
                toggleSkins();
            } else {
                print("Modern Pinch Button ignored: Y2K skin is already active.");
            }
        };
