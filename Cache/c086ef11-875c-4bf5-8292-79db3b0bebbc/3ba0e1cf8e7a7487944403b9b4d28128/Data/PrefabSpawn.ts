...
import { BoxCollider } from 'SpectaclesInteractionKit/Components/Colliders/BoxCollider';
import { ObjectPrefab } from 'SpectaclesInteractionKit/Core/Assets/ObjectPrefab';
...
export class PrefabObjectSpawner extends BaseScriptComponent {
    ...
    private spawnObject(): void {
        ...
        const newObject = global.scene.createObjectFromPrefab(this.prefabTemplate);
        ...
        newObject.setParent(global.scene.getRootObject());
    ...
    private setupInteraction(object: SceneObject): void {
        ...
        if (!hasCollider) {
            const collider = object.addComponent(BoxCollider);
            if (collider) {
        ...
        if (!hasInteractable) {
            const interactable = object.addComponent(Interactable);
            if (interactable) {
        ...
        if (!hasManipulation) {
            const manipulation = object.addComponent(InteractableManipulation);
            if (manipulation) {
