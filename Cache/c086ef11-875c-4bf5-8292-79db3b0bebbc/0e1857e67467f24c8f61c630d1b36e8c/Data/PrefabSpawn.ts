...
export class PrefabSpawner extends BaseScriptComponent {
    ...
    onAwake(): void {
        ...
        if (typeof this.instantiator.notifyOnReady === 'function') {
            this.instantiator.notifyOnReady(() => {
                this.isInitialized = true;
                print('Instantiator is ready. Prefab spawner initialized.');
            });
        } else {
            print('Warning: notifyOnReady is not a function on instantiator. Skipping initialization.');
            this.isInitialized = true;
        }
    ...
    onDestroy(): void {
        if (this.pinchButton && this.onPinchCallback && typeof this.pinchButton.onButtonPinched?.remove === 'function') {
